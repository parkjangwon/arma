...
설치 스크립트는 **2가지 스코프**를 지원한다.

- user 스코프(기본, sudo 없음): 일반 계정 설치/운용
- system 스코프(sudo): 시스템 전역 설치/운용

user 스코프 설치 (권장: 개인 개발 환경)

```bash
curl -fsSL https://raw.githubusercontent.com/parkjangwon/arma/main/install.sh | bash -s -- --scope user --with-systemd
```

system 스코프 설치 (운영 서버/전역 설치)
...
```bash
curl -fsSL https://raw.githubusercontent.com/parkjangwon/arma/main/install.sh | bash -s -- --scope user --dry-run --with-systemd
```
...
주의: 삭제는 해당 스코프의 ARMA 바이너리/서비스와 설정 디렉토리를 함께 제거한다.
